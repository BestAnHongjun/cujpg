...
#define NPP_CHECK_CUDA(S) do {cudaError_t eCUDAResult; \
        eCUDAResult = S; \
        if (eCUDAResult != cudaSuccess) std::cout << "NPP_CHECK_CUDA - eCUDAResult = " << eCUDAResult << ", Error at " << __FILE__ << ", " << __LINE__ << std::endl << cudaGetErrorString(eCUDAResult) << std::endl; \
